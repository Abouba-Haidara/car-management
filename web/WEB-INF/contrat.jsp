...
<body class="fixed-nav sticky-footer bg-dark" id="page-top" ng-controller="locationCtrl">
  ...
  <div class="content-wrapper">
    <div class="container-fluid">
      ...
      <div class="card-header">Redaction  d'un contrat</div>
      <div class="card-body">
           <c:if test="${exception}">
                   <span class="text-danger">${exception}</span> 
            </c:if>
          <form name="loc" action="${pageContext.request.contextPath}/location?action=savecontrat" method="post">
           <div class="form-group">
               <label for="idv" class="text-primary"><c:choose ><c:when test="${vid==true}">Aucune location ne prise pour le moment </c:when> <c:when test="${vid==false}"> Location En Cours </c:when> </c:choose></label>                
               <select name="idlocation" id="idv" class="form-control" ng-model="id" <c:if test="${vid}"> disabled </c:if> required ng-change="findLocation()">
                   <option value=""></option>            
                   <c:forEach items="${lists}" var="l">                             
                        <option value="${l.idLocation.id }"> ${l.idLocation.matricule }                       
                    </option>
                 </c:forEach>
              </select>    
                 <c:if test="${vid}">
                     <span class="text-danger">Aucune location prise <a  href="${pageContext.request.contextPath}/location?action=effectuer" class="btn btn-sm btn-primary  fa fa-edit"></a></span> 
                 </c:if>
          </div>  
          <div class="form-group">
            <div class="form-row">
              <div class="col-md-6">
                ...
                <input class="form-control" id="n" type="text" readonly ng-model="nom"  placeholder="Nom Locataire">
              </div>
              <div class="col-md-6">
                <label for="p">Prenom </label>
                <input class="form-control" id="p" type="text" readonly ng-model="prenom" placeholder="Prenom Locataire">
              </div>
            </div>
          </div>
          ...
          <div class="form-group">
            <div class="form-row">
              <div class="col-md-6">
                ...
                <input class="form-control" id="dd" name="dd" ng-model="dd"  value="" readonly type="text" >
       
              </div>
              <div class="col-md-6">
                <label for="dr">Date  fin</label>
                <input class="form-control" id="dr"  ng-model="df" name="df"  readonly type="text">
              </div>
            </div>
          </div>
          <div class="form-group">
              <label for="marque">Voiture</label>
                   <div class="form-row">                      
                          <div class="col-md-6">
                            ...
                            <input class="form-control" id="marque" type="text" readonly ng-model="marque"  placeholder="Marque">
                          </div>
                          <div class="col-md-6">
                            <label for="model">Modele</label>
                            <input class="form-control" id="model" type="text" readonly ng-model="modele" placeholder="Modele">
                          </div>
                     </div>
          </div>
        <div class="form-group">
             <div class="col-md-12">             
              <label for='motif'>Motif du location</label>
              <textarea class="form-control" id="motif" required ng-model="motif"></textarea>
              </div>
          </div>
            <button class="btn btn-primary btn-block" type="submit" ng-disabled="loc.$invalid">Géneré</button>
        </form>  
      </div>
      </div>
    </div>
